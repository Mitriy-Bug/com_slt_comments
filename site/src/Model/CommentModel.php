...
class CommentModel extends ItemModel
{
	...
	protected function populateState()
	{
		...
		if ((!$user->authorise('core.edit.state', 'com_slt_comments')) && (!$user->authorise('core.edit', 'com_slt_comments')))
		{
		...
		{
			$id = Factory::getApplication()->getUserState('com_slt_comments.edit.comment.id');
		}
		else
		{
			$id = Factory::getApplication()->input->get('id');
			Factory::getApplication()->setUserState('com_slt_comments.edit.comment.id', $id);
		}
		...
		if ($this->_item === null)
		{
			...
			if ($table && $table->load($id))
			{
				...
				if ($published = $this->getState('filter.published'))
				{
					...
					{
						throw new \Exception(Text::_('COM_SLT_COMMENTS_ITEM_NOT_LOADED'), 403);
					}
				}
			...
			if (empty($this->_item))
			{
				throw new \Exception(Text::_('COM_SLT_COMMENTS_ITEM_NOT_LOADED'), 404);
			}
		}
